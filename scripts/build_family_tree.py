...
function layout(){
  ...
  function podMembers(pid){
    ...
    partnerUnions(pid).forEach(function(u){
      U[u].partners.forEach(function(x){
        if(x!==pid&&!done[x]&&m.indexOf(x)<0&&!BIRTH[x]&&!hidden[x])m.push(x);
      });
    });
    return m;
  }
  ...
  Object.keys(P).filter(function(p){return !BIRTH[p]&&!hidden[p];})
...
function fitTo(ids,pad,ms){
  ...
  k=Math.max(.12,Math.min(k,1.55));
  animateTo(r.width/2-((b.x0+b.x1)/2)*k, r.height/2-((b.y0+b.y1)/2)*k, k, ms);
}
function fitAll(ms){fitTo(null,.94,ms);}
...
function select(id,opts){
  opts=opts||{};
  if(id&&S.sel===id&&!opts.keep){S.sel=null;applyEmphasis();if(S.focus)fitAll();return;}
  S.sel=id;
  applyEmphasis();
  if(id){
    if(S.focus){
...
bind('reset',function(){S.sel=null;S.open={};applyEmphasis();closeProfile();fitAll();});
bind('me',function(){
...
requestAnimationFrame(function(){fitAll(1);});
window.addEventListener('resize',function(){if(S.view==='tree')fitAll(260);});
